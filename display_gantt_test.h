...
const char* TASKS[] = { "FindBank", "GetMortgage", "DrawDown", "BuildFoundation", "BuildWalls",
                  "RoofAndCeiling", "Plumbing", "Electrics", "InspectBuild", "Snagging"}; //Example tasks array

void display_gantt(struct Task task, int task_index);
...
void display_gantt_test() {
    struct Task test_data[10] = { //set structure. name is initialized to empty string.
            ...
            {.name = "", .start_month = 9, .end_month = 10, .num_dependencies = 2, .dependencies = {0, 7, 8}},
    };

    for (int i = 0; i < (sizeof(test_data) / sizeof(test_data[0])); i++) { //strcpy to copy constant example tasks array
        strcpy(test_data[i].name, TASKS[i]); //to structure.
    }

    gantt_static(); //call function that prints static features of gantt chart (months, dependencies, cells)

    for (int i = 0; i < sizeof(test_data) / sizeof(test_data[0]); i++) { //loop to print tasks
        int start = test_data[i].start_month - 1; //making value match with enums (0-11)
        ...
        for (int j = 1; j <= test_data[i].num_dependencies; j++) { //print dependencies (j = 1 so dependency number correlates
            printf(" %d", test_data[i].dependencies[j]); //to task number (1-9)
        }
